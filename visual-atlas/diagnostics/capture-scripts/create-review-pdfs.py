...
def create_review(name: str, profile: str, page_size: tuple[int, int], image_size: tuple[int, int], origin: tuple[int, int]) -> None:
    ...
    for locale, label in LOCALES:
        for surface in SURFACES:
            ...
            heading = f"CHG-199 Review | {surface} | {label} | {profile}"
            heading_size = min(15, (page_size[0] - 2 * x) * 15 / stringWidth(heading, "Helvetica-Bold", 15))
            pdf.setFont("Helvetica-Bold", heading_size)
            pdf.drawString(x, page_size[1] - 24, heading)
            pdf.setFillColorRGB(0.31, 0.36, 0.43)
            pdf.setFont("Helvetica", 8)
            pdf.drawString(x, page_size[1] - 38, "Candidate 8a9ac0104190144110d335c93eb11f8c51f6a898 | Golden UI v2 | awaiting independent pixel audit")
