...
async function saveCapture(page, locale, viewport, surface) {
  ...
  if (surface === "logic-registry") {
    const instruction = locale.key === "ko"
    ...
    const layout = await subtitle.evaluate(element => {
      ...
      return {
        ...
        renderedLineCount: measureLines(element),
        fullLineCount,
    ...
    const done = await dialog.getByRole("button", { name: locale.key === "ko" ? "닫기" : "Done", exact: true }).boundingBox();
    if (layout.text !== instruction || layout.lineClamp !== "8" || layout.renderedLineCount !== layout.fullLineCount || layout.fullLineCount > 8 || layout.scrollWidth > layout.width) throw new Error(`Registry instruction is clipped or overflowing at ${locale.key} ${viewport.profile}: ${JSON.stringify(layout)}`);
    if (viewport.width >= 412 && layout.fullLineCount > 4) throw new Error(`Registry instruction wraps too much at ${locale.key} ${viewport.profile}: ${JSON.stringify(layout)}`);
    if (!done || done.x < 0 || done.y < 0 || done.x + done.width > viewport.width || done.y + done.height > viewport.height) throw new Error(`Registry Done control is outside the viewport at ${locale.key} ${viewport.profile}.`);
  }
...
await writeFile(resolve(root, "capture-metrics.json"), `${JSON.stringify({ candidate: "8a9ac0104190144110d335c93eb11f8c51f6a898", tree: "dc99438cdcf8e2f737e5de87d6afc550d4008f24", reducedMotion: "reduce", captures: captured }, null, 2)}\n`);
