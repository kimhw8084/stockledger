...
import { chromium } from "/Users/haewonkim/.codex-fabric/worktrees/CF-e2b5042151557732c80eb1ca/node_modules/playwright/index.mjs";

const root = "/Users/haewonkim/.codex-fabric/runs/stockledger/stockledger-prod-c16-authoring-config-error-responsive-fix-v2/visual-atlas";
...
try {
  {
    ...
    try {
      ...
      const stockError = dialog.getByText("Select a stock.", { exact: true });
      await stockError.waitFor({ state: "visible" });
      await dialog.getByTestId("eye-stock").scrollIntoViewIfNeeded();
      await stockError.scrollIntoViewIfNeeded();
      const stockFieldBounds = await dialog.getByTestId("eye-stock").boundingBox();
      const stockErrorBounds = await stockError.boundingBox();
      if (!stockFieldBounds || !stockErrorBounds || stockFieldBounds.y < 0 || stockErrorBounds.y < 0 || stockFieldBounds.y + stockFieldBounds.height > 900 || stockErrorBounds.y + stockErrorBounds.height > 900) throw new Error("Eye required Stock field and adjacent error are outside the capture frame.");
      await capture(page, "eye-blank-validation", "The first required Stock field and its adjacent actionable error are visible with composer context.");
...

  {
    ...
    try {
      ...
      const eyeContextError = dialog.getByText("Select an Eye.", { exact: true });
      await eyeContextError.waitFor({ state: "visible" });
      await dialog.getByTestId("journal-eye").scrollIntoViewIfNeeded();
      await eyeContextError.scrollIntoViewIfNeeded();
      const eyeFieldBounds = await dialog.getByTestId("journal-eye").boundingBox();
      const eyeErrorBounds = await eyeContextError.boundingBox();
      if (!eyeFieldBounds || !eyeErrorBounds || eyeFieldBounds.y < 0 || eyeErrorBounds.y < 0 || eyeFieldBounds.y + eyeFieldBounds.height > 900 || eyeErrorBounds.y + eyeErrorBounds.height > 900) throw new Error("Journal required Eye context and adjacent error are outside the capture frame.");
      for (const futureError of ["Choose the action you recorded.", "Choose your thesis assessment.", "Choose your timing assessment."]) {
        if (await dialog.getByText(futureError, { exact: true }).count()) throw new Error(`Journal exposed a future-step validation error before its step: ${futureError}`);
      }
      await capture(page, "journal-required-validation", "The current required Eye context and adjacent error are visible; later action, thesis, and timing errors remain hidden.");
...

  {
    ...
    try {
      ...
      await dialog.getByText("Notification preferences could not be saved. No changes were made.", { exact: true }).waitFor({ state: "visible" });
      const renderedText = await page.locator("body").innerText();
      if (renderedText.includes("Uncaught exception in event handler.")) throw new Error("Raw notification persistence exception appeared in rendered user-facing state.");
      await page.getByText("Enabled", { exact: true }).waitFor({ state: "visible" });
      await capture(page, "notification-save-error", "Injected technical exception retains localized safe copy, the unchanged Enabled preference, and retry/cancel actions.");
      await page.evaluate(() => {
        const prototype = IDBObjectStore.prototype;
...
await writeFile(resolve(root, "state-evidence.json"), `${JSON.stringify({ candidate: "8a9ac0104190144110d335c93eb11f8c51f6a898", tree: "dc99438cdcf8e2f737e5de87d6afc550d4008f24", reducedMotion: "reduce", states }, null, 2)}\n`);
