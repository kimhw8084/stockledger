...
const repo = "/Users/haewonkim/.codex-fabric/worktrees/CF-e2b5042151557732c80eb1ca";
const diagnostics = join(root, "diagnostics");
const captureScripts = join(diagnostics, "capture-scripts");
const candidate = "8a9ac0104190144110d335c93eb11f8c51f6a898";
const tree = "dc99438cdcf8e2f737e5de87d6afc550d4008f24";
...
const manifest = {
  project: "stockledger",
  change: "CHG-199",
  request: "stockledger-prod-c16-authoring-config-error-responsive-fix-v2",
  operation: "FIX",
...
const report = {
  ...
  sharedImpact: "WindowPanel keeps its two-line subtitle default and allows the Raw Data Registry to show its full instruction in eight lines. Primary-route and deep-review/detail regressions pass. Notification preference failures use localized safe copy and stay out of the global error banner.",
...
if (productChangedFiles.length !== 15) throw new Error(`Unexpected Product changed-file count: ${productChangedFiles.length}`);
const sourceImpact = {
  candidate: { commit: candidate, tree, base },
  productCommitCount: 2,
  ...
  featureOwnership: {
    ...
    notificationConfirmation: "NotificationSettingsPanel uses fixed localized failure copy; the notification preference commit suppresses the global raw exception banner while preserving pending, success-only close, persisted truth, retry, and cancel behavior",
  },
  architectureChecks: {
  ...
  sharedPrimitiveImpact: {
    windowPanel: "two-line subtitle default preserved; Raw Data Registry opts into eight lines; shared focus, Escape, close, safe-area, and reduced-motion behavior exercised by release browser suite",
...
const taskCoverage = {
  ...
  surfaces: [
    ...
    { surface: "logic-registry", oracle: "Open; inspect priceHistorySeries identifier/meaning/importance/source/coverage/downstream processing; verify the complete EN/KO instruction wraps without clipping at all five supported viewports; supported help; close and exact focus return.", verifiedBy: "tests/e2e/chg199-authoring-config.spec.ts", states: ["ready", "field-detail", "help", "escape-focus", "responsive-copy"], screenshots: [stateByName["registry-field-detail"], stateByName["registry-help"]] },
    { surface: "notification-confirmation", oracle: "Disable; cancel without mutation and restore focus; reopen; pending state; successful persistence; injected technical exception renders only localized safe copy, retains the prior preference, and permits retry/cancel.", verifiedBy: "tests/e2e/chg199-authoring-config.spec.ts + tests/e2e/workspace.spec.ts + state capture fixture", states: ["cancel", "pending-local-persistence-ack", "success", "error-safe-copy-retry", "escape-focus"], screenshots: [stateByName["notification-confirmation-cancel"], stateByName["notification-canceled"], stateByName["notification-pending"], stateByName["notification-save-error"], stateByName["notification-save-success"]] },
  ],
...
await writeDiagnostic("primary-route-regression.json", {
  ...
  result: "PASS",
  candidate,
  fullPlaywright: { passed: 77, skipped: 1, failed: 0 },
...
const qualification = {
  ...
  qualification: {
    ...
    playwright_e2e: "PASS; 77 passed, 1 existing mobile hardware-keyboard skip, 0 failed",
