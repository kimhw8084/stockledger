...
The repository drill is `npm run recovery:drill`. It proves the local consistent-snapshot boundary, authored/durable-state retention, validated export parsing and rejection/isolation of truncated/incompatible candidates. Its measured elapsed time is an engineering qualification result only; it is not an RTO/SLA.
...
The corresponding local/tabletop checks are retained in focused tests and generated CHG-96 evidence. No external pager, hosted mail delivery, commercial provider, billing service or real incident response is claimed.
