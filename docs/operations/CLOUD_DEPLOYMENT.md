...
- Notification preferences are deliberately device-local in CHG-94: they round-trip through the complete personal workspace backup but are not included in `stockledger-personal-sync-v1`. No account-wide destination, provider credential, delivery receipt or service cancellation is implied by CHG-93. If a future deployment makes intents account-owned, it must add owner-derived RLS, exact-version preference conflict handling, an additive migration and a verified deletion worker that cancels only those account-owned pending/retry intents; this repository does not claim that hosted service.
