...
- Release proof requires an explicit trusted protected-base SHA (or `STOCKLEDGER_PROTECTED_BASE`); it must be an existing ancestor of the exact candidate source. Pull-request and push-to-main CI bind their authoritative event base/prior-target facts, while local and manually dispatched proof fail closed without an explicit base. The recorded R1 historical base remains historical evidence, not a standing release default.
- This execution did not claim a full-stack cloud pass: `npm run test:cloud` could not inspect the disposable Supabase stack because Docker was unavailable at `unix:///Users/haewonkim/.orbstack/run/docker.sock`, and local database lint could not connect to `127.0.0.1:55422`. The repository-native `cloud-contract` PR job remains the authoritative full-stack test/lint path.
