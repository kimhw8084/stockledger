...
export function NotificationSettingsPanel({ data, actions, language }: { data: AppData; actions: ReturnType<typeof useAppModel>["actions"]; language: AppLanguage }) {
  ...
  return <Card>
    <View style={styles.panel}>
      ...
      {pendingWorkerHandoff ? <Text accessibilityLiveRegion="polite" style={styles.pending}>{t(language, "settings.notifications.pendingWorkerHandoff")}</Text> : null}
      {workerStatus ? <Text accessibilityLiveRegion="polite" style={styles.detail}>{t(language, "settings.notifications.lastKnownState", { state: t(language, workerStateKey), at: workerStatus.generatedAt, attempts: workerStatus.attemptCount })}{workerStatus.errorClass ? ` ${workerStatus.errorClass}` : ""}</Text> : <Text style={styles.detail}>{t(language, "settings.notifications.noLastKnownState")}</Text>}
