...
const snapshotSchema = z.looseObject({
  provenance: opt(z.looseObject({ schemaVersion: z.literal(2), origin: z.enum(["provider", "import", "demo"]), observedDate: text, retrievedAt: timestamp, currency: z.literal("USD"), adjustment: z.enum(["adjusted", "unadjusted", "unknown"]), datasetId: id,
    providerIdentity: opt(text), providerProductId: opt(text), datasetCategory: opt(text), sourceRequestIdentity: opt(text), freshnessState: opt(text), coverageState: opt(text), contentHash: opt(text), rightsProfileId: opt(text), validationIssues: opt(strings), failureClass: opt(text) })),
