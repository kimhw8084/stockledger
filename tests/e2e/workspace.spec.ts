...
test("records a deliberate decision, reviews its outcome, and preserves an amendment", async ({ page }) => {
  ...
  await page.getByRole("dialog").last().getByRole("radio", { name: "Entered", exact: true }).click();
  await page.getByRole("textbox", { name: "Why did you act this way?" }).fill("Regression review: wait for confirmed evidence.");
  ...
  await page.getByRole("radio", { name: "Early", exact: true }).click();
  await page.getByRole("button", { name: "Save Decision", exact: true }).click();
  ...
  await page.getByRole("textbox", { name: "Why did you act this way?" }).fill("Regression amendment: reviewed the source.");
...
test("keeps notification consent explicit and exposes device-local delivery status", async ({ page }) => {
  ...
  await expect(confirmation.getByText(/saves your opt-out.*allowed delivery channels/)).toBeVisible();
